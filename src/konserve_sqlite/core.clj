(ns konserve-sqlite.core
  "SQLite store implemented with HugSQL."
...
(hugsql/def-db-fns "konserve_sqlite/db.sql" {:adapter (kebab-adapter)})
...
(defrecord SqliteStore [db serializer read-handlers write-handlers locks]
  PEDNAsyncKeyValueStore
  (-exists? [this key]
    (let [id (str (uuid key))]
...
(defn new-sqlite-store
  "Constructs a SQLite store either with URL for db or a SQLite connection
  ...
  (let [db (if (string? db) (connection-uri db) db)]
    (go (try
          (create-table! db)
          (map->SqliteStore {:db db
                         :serializer serializer
                         :read-handlers read-handlers
                         :write-handlers write-handlers
                         :locks (atom {})})
          (catch Exception e
            (ex-info "Cannot open SQLite database."
                     {:type :db-error
                      :db db
                      :exception e}))))))
